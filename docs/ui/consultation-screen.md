...
O atalho de download aponta diretamente para o Setup da versão canônica atualmente publicada (`v0.0.6`). O teste `apps/web/tests/settings-panel.test.ts` cruza a URL do Setup com `Directory.Build.props`, para que um futuro bump de versão não deixe o link silenciosamente desatualizado.
