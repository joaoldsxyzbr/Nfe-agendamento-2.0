...
- **Release pública atual:** `0.0.25`; publicada somente a partir do mesmo SHA validado pelo CI completo.
...
A v0.0.22 adicionou `/extension-test.html`, um gate isolado que não usa `BridgeClient` nem `127.0.0.1:17345`. A v0.0.25 publica a extensão 0.2.3 com compatibilidade do `supplier-rules.json` legado, detecção reativa da ponte, recuperação após cold start e diagnóstico mais preciso do Portal. O gate existe somente para validar fisicamente site + extensão + Portal + A1 antes da remoção do Bridge do fluxo principal.
...
Versão canônica da release: **v0.0.25**.
...
A release também publica `NFeAgendamento-Extension-v0.2.3.zip` para o piloto Chromium e o gate isolado sem Bridge. O Microsoft Edge WebView2 Runtime continua necessário apenas para o fallback pelo helper Portal/WebView2.
...
O CI não consegue provar interação real com certificado A1, SEFAZ, Portal/hCaptcha ou uma impressora específica. Para declarar a v0.0.25 fisicamente validada, executar:
