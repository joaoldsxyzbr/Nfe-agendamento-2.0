...
- **Release pública atual:** `0.0.24`; publicada somente a partir do mesmo SHA validado pelo CI completo.
...
- HEAD pós-v0.0.24 prepara a extensão 0.2.3 com reconciliação de operação em `storage.session`, start idempotente após resposta perdida, detecção de popup/aba obsoletos e status explícito para evitar espera indefinida;\n- importação `supplier-rules.json` compatível com o casing legado do Bridge, saída canônica, erros com caminho do campo e armazenamento local restrito a contextos confiáveis da extensão;
...
A v0.0.22 adicionou `/extension-test.html`, um gate isolado que não usa `BridgeClient` nem `127.0.0.1:17345`. A v0.0.24 atualiza o piloto para a extensão 0.2.2, corrige a compatibilidade do `supplier-rules.json` legado e adiciona detecção reativa quando a ponte da extensão fica pronta. O gate existe somente para validar fisicamente site + extensão + Portal + A1 antes da remoção do Bridge do fluxo principal.
...
Versão canônica da release: **v0.0.24**.
...
A release também publica `NFeAgendamento-Extension-v0.2.2.zip` para o piloto Chromium e o gate isolado sem Bridge. O Microsoft Edge WebView2 Runtime continua necessário apenas para o fallback pelo helper Portal/WebView2.
...
O CI não consegue provar interação real com certificado A1, SEFAZ, Portal/hCaptcha ou uma impressora específica. Para declarar a v0.0.24 fisicamente validada, executar:
