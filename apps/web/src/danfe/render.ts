import { resolveSupplierProduct } from '../nfe/product-mapping';
...
function buildProductsTable(nfe: ParsedNfe, products: readonly ParsedNfeProduct[]): string {
  const rows = products.map((product) => {
    const mapping = resolveSupplierProduct({ supplierRuleId: nfe.supplierRuleId, emitterName: nfe.issuer.name, xProd: product.description, cProd: product.code });
    ...
    const internalQuantity = resolveSupplierInternalQuantity({ supplierRuleId: nfe.supplierRuleId, emitterName: nfe.issuer.name, quantity: product.quantity });
